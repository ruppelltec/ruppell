...
const CoursesPage = () => {
    ...
    // Read search term and category from URL query parameters
    ...
    const filteredCourses = useMemo(() => {
        return allCourses.filter((course) => {
            // 1. Search Term (Title only)
            if (searchTerm && !course.title.toLowerCase().includes(searchTerm.toLowerCase())) {
                return false;
            }

            // 1b. Expert Search
            if (expertSearch && !course.expert.toLowerCase().includes(expertSearch.toLowerCase())) {
                return false;
            }
    ...
    // No manual columns needed for simple grid
            ...
            <div className="bg-deep-black-500 pt-[120px] pb-10 px-4 mb-8 text-center relative overflow-hidden">
                <div className="relative z-10 container mx-auto">
                    ...
                    <p className="text-gray-400 text-lg mb-8 max-w-2xl mx-auto">
                        Explora una selección curada de cursos impartidos por expertos de clase mundial.
                    </p>
                    ...
                    <div className="flex flex-wrap gap-2 justify-center max-w-4xl mx-auto">
                        {selectedCategories.map(catId => {
                            ...
                            return cat ? (
                                <button key={catId} onClick={() => toggleCategory(catId)} className="bg-white/10 backdrop-blur-md text-white border border-white/20 px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-2 hover:bg-white/20 transition">
                                    Nido: {cat.title} <span className="opacity-70">×</span>
                                </button>
                            ) : null;
                        })}
                        {selectedLevels.map(level => (
                            <button key={level} onClick={() => toggleLevel(level)} className="bg-white/10 backdrop-blur-md text-white border border-white/20 px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-2 hover:bg-white/20 transition">
                                Nivel: {level} <span className="opacity-70">×</span>
                            </button>
                        ))}
                        {priceRange[1] < 2000000 && (
                            <button onClick={() => setPriceRange([0, 2000000])} className="bg-white/10 backdrop-blur-md text-white border border-white/20 px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-2 hover:bg-white/20 transition">
                                Max ${new Intl.NumberFormat('es-CO').format(priceRange[1])} <span className="opacity-70">×</span>
                            </button>
                        )}
                        {maxDuration < 160 && (
                            <button onClick={() => setMaxDuration(160)} className="bg-white/10 backdrop-blur-md text-white border border-white/20 px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-2 hover:bg-white/20 transition">
                                Max {maxDuration}h <span className="opacity-70">×</span>
                            </button>
                        )}
                        {minRating > 0 && (
                            <button onClick={() => setMinRating(0)} className="bg-white/10 backdrop-blur-md text-white border border-white/20 px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-2 hover:bg-white/20 transition">
                                &gt; {minRating} ★ <span className="opacity-70">×</span>
                            </button>
                        )}
                        {expertSearch && (
                            <button onClick={() => setExpertSearch("")} className="bg-white/10 backdrop-blur-md text-white border border-white/20 px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-2 hover:bg-white/20 transition">
